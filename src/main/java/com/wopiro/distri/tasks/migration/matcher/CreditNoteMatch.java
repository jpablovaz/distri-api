...
//				document = RepReference.getCreditNoteService().save(document);
//			}
			DocumentOrder order = new DocumentOrder(DocType.CREDIT_NOTE, idOld, document.getDateTime());
			MemoryData.getDocumentsOrder().add(order);
			MemoryData.getCreditNotes().put(idOld, document);
		}
	}
